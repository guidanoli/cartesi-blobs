...
function getAccount() {
    ...
    if (mnemonic !== undefined) {
        ...
        if (!isHex(privateKey)) {
            throw new Error("Environment variable PRIVATE_KEY must be in hexstring format");
        }
        return privateKeyToAccount(privateKey);
    } else {
        throw new Error("Expected either environment variables MNEMONIC or PRIVATE_KEY to be set");
    }
}
